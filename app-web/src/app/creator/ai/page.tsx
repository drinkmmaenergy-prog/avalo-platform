...
function BotCard({
  ...
  return (
    <button
      onClick={onClick}
      className="card overflow-hidden hover:ring-2 hover:ring-purple-500/50 transition-all duration-200 group text-left w-full"
    >
...
export default function CreatorAIPage() {
  ...
  const handleProfilePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_FILE_SIZE_BYTES) {
      alert('File too large. Maximum 100MB per file.');
      return;
    }
  ...
  const handleGalleryPhotosChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (oversized.length > 0) {
      alert(`${oversized.length} file(s) exceed 100MB limit and were skipped.`);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Upload profile photo first (becomes photos[0])
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 p-6 mb-8">
            ...
            <div className="space-y-5">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                  Gallery Photos{' '}
                  <span className="text-xs text-gray-400">(no limit, 100MB per file)</span>
                </label>
