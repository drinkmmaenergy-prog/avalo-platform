...
$BackupRoot = Join-Path $AuditRoot ('batchK2v2-safety-' + $timestamp)
...
if (Test-Path -LiteralPath $path) {
  ...
  $new = $new.Replace('Promotion campaign', 'Sponsored placement campaign')
  $new = $new.Replace('promotion campaign', 'sponsored placement campaign')
  Save-IfChanged -Path $path -Old $raw -New $new
}
...
# digital-goods.tsx - EN only, safe
...
$reportPath = Join-Path $AuditRoot ('batchK2v2-report-' + $timestamp + '.txt')
@(
  'Batch K2 v2 complete.'
