...
TARGET_LOCALES = [
    ...
    "cs","sk","hu","ro","bg","hr","sl","el","uk","ru","lt","lv","et",
    "sr","bs","mk","sq",
    # Key Asia
    "ja","ko","zh","hi","th","vi","id","ms","tr"
]
...
def translate_text(text: str, target_locale: str) -> str:
    ...
    effective_target = TARGET_MAP.get(target_locale, target_locale)
    ...
    try:
        shorter = protected.replace(" I understand that I can withdraw consent at any time.", "")
        translator = GoogleTranslator(source="en", target=effective_target)
        translated = translator.translate(shorter)
        if translated:
            translated = restore_placeholders(translated, mapping)
            if target_locale == "hr":
                return translated + " Razumijem da mogu povući privolu u bilo kojem trenutku."
            if target_locale == "bs":
                return translated + " Razumijem da mogu povući saglasnost u bilo kojem trenutku."
            if target_locale == "sr":
                return translated + " Razumem da mogu da povučem saglasnost u bilo kom trenutku."
            if target_locale == "sl":
                return translated + " Razumem, da lahko kadar koli prekličem soglasje."
            return translated + " " + "I understand that I can withdraw consent at any time."
    except Exception:
        pass
...
def main() -> None:
    ...
    write_json(AUDIT_OUT / "legal_i18n_eu_asia_audit_before.json", audit_before)
    ...
    for idx, locale in enumerate(TARGET_LOCALES, start=1):
        print(f"[{idx}/{len(TARGET_LOCALES)}] START {locale}", flush=True)
        try:
            ...
            write_json(AUDIT_OUT / "legal_i18n_eu_asia_progress.json", progress)
            print(f"[{idx}/{len(TARGET_LOCALES)}] DONE {locale}", flush=True)
        except Exception as e:
            errors.append({"locale": locale, "error": repr(e)})
            write_json(AUDIT_OUT / "legal_i18n_eu_asia_errors.json", errors)
            print(f"[{idx}/{len(TARGET_LOCALES)}] ERROR {locale}: {e}", flush=True)
            raise

    apply_report = {
        "version": "final-eu-asia",
        "target_locales": TARGET_LOCALES,
        ...
        "notes": [
            ...
            "Priority rollout: Europe + selected Asia.",
            ...
            "Progress log written per locale."
        ]
    }
    write_json(AUDIT_OUT / "legal_i18n_eu_asia_apply_report.json", apply_report)
