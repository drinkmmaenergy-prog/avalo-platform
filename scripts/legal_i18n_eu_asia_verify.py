...
TARGET_LOCALES = [
    ...
    "sr","bs","mk","sq","ja","ko","zh","hi","th","vi","id","ms","tr"
]
...
FORBIDDEN_VALUES = ["TODO", "lorem ipsum"]
...
def scan_repo_text() -> list[dict]:
    ...
    for path in REPO.rglob("*"):
        if not path.is_file():
            continue
        if any(part in {".git", "node_modules", ".next", "dist", "build", "android", "ios", ".gradle", "Pods", "audit-out", "docs", "backup", "backups"} for part in path.parts):
            continue
...
def main() -> None:
    ...
    out = AUDIT_OUT / "legal_i18n_eu_asia_verify_report.json"
