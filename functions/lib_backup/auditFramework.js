...
exports.testComplianceControlV1 = exports.monthlyComplianceReviewScheduler = exports.getCertificationStatusV1 = exports.runAccessibilityAuditV1 = exports.generateSOC2ReportV1 = exports.generateISO27001ReportV1 = exports.CertificationStatus = exports.ControlStatus = void 0;
...
(function (ControlStatus) {
    ...
    ControlStatus["PARTIALLY_IMPLEMENTED"] = "partially_implemented";
    ControlStatus["NOT_IMPLEMENTED"] = "not_implemented";
...
(function (CertificationStatus) {
    ...
    CertificationStatus["READY_FOR_AUDIT"] = "ready_for_audit";
...
const ISO27001_CONTROLS = [
    {
        ...
        owner: "Security Team",
        lastReviewed: firestore_1.Timestamp.now(),
        nextReview: firestore_1.Timestamp.fromMillis(Date.now() + 365 * 24 * 60 * 60 * 1000),
    },
    {
        controlId: "A.8.2",
        ...
        lastReviewed: firestore_1.Timestamp.now(),
        nextReview: firestore_1.Timestamp.fromMillis(Date.now() + 180 * 24 * 60 * 60 * 1000),
    },
    {
        controlId: "A.8.24",
...
const SOC2_CONTROLS = [
    {
        ...
        status: ControlStatus.IMPLEMENTED,
        lastTested: firestore_1.Timestamp.now(),
        nextTest: firestore_1.Timestamp.fromMillis(Date.now() + 90 * 24 * 60 * 60 * 1000),
    },
    {
        controlId: "CC7.2",
        trustServiceCriteria: "Security",
    ...
    },
    {
        ...
        status: ControlStatus.IMPLEMENTED,
        lastTested: firestore_1.Timestamp.now(),
        nextTest: firestore_1.Timestamp.fromMillis(Date.now() + 30 * 24 * 60 * 60 * 1000),
    },
];
// ============================================================================
 ...
 * @auth admin
 */
exports.generateISO27001ReportV1 = (0, https_1.onCall)({
    region: "europe-west3",
    cors: true,
    ...
    v2_1.logger.info("Generating ISO 27001 compliance report");
    // Calculate statistics
    const totalControls = ISO27001_CONTROLS.length;
    const implemented = ISO27001_CONTROLS.filter((c) => c.status === ControlStatus.IMPLEMENTED).length;
    const partiallyImplemented = ISO27001_CONTROLS.filter((c) => c.status === ControlStatus.PARTIALLY_IMPLEMENTED).length;
    const notImplemented = ISO27001_CONTROLS.filter((c) => c.status === ControlStatus.NOT_IMPLEMENTED).length;
    const notApplicable = ISO27001_CONTROLS.filter((c) => c.status === ControlStatus.NOT_APPLICABLE).length;
    const complianceScore = Math.round(((implemented + partiallyImplemented * 0.5) / (totalControls - notApplicable)) * 100);
    // Identify gaps
    const findings = [];
    const notImplementedControls = ISO27001_CONTROLS.filter((c) => c.status === ControlStatus.NOT_IMPLEMENTED);
    ...
    const partialControls = ISO27001_CONTROLS.filter((c) => c.status === ControlStatus.PARTIALLY_IMPLEMENTED);
    ...
    v2_1.logger.info(`ISO 27001 report generated: ${complianceScore}% compliance`);
    return report;
});
/**
 ...
 * @auth admin
 */
exports.generateSOC2ReportV1 = (0, https_1.onCall)({
    region: "europe-west3",
    cors: true,
    enforceAppCheck: true,
    timeoutSeconds: 300,
    ...
    v2_1.logger.info("Generating SOC 2 compliance report");
    const totalControls = SOC2_CONTROLS.length;
    const implemented = SOC2_CONTROLS.filter((c) => c.status === ControlStatus.IMPLEMENTED).length;
 ...
 * @auth admin
 */
exports.runAccessibilityAuditV1 = (0, https_1.onCall)({
    region: "europe-west3",
    cors: true,
    enforceAppCheck: true,
}, async (request) => {
    ...
    v2_1.logger.info("Running WCAG 2.2 accessibility audit");
    // Categorize criteria
    ...
    const calcCategoryScore = (criteria) => {
    ...
    const overallScore = Math.round((breakdown.perceivable.score +
        breakdown.operable.score +
        breakdown.understandable.score +
        breakdown.robust.score) / 4);
    const level = overallScore >= 95 ? "AAA" : overallScore >= 90 ? "AA" : "A";
    ...
    await db.collection("accessibility_audits").add({
        ...result,
        auditedBy: userId,
        auditedAt: firestore_1.FieldValue.serverTimestamp(),
    });
    v2_1.logger.info(`WCAG audit complete: ${overallScore}/100 (Level ${level})`);
    return result;
});
/**
 ...
 * @auth admin
 */
exports.getCertificationStatusV1 = (0, https_1.onCall)({
    region: "europe-west3",
    cors: true,
    enforceAppCheck: true,
}, async (request) => {
    ...
    const iso27001Score = Math.round((ISO27001_CONTROLS.filter((c) => c.status === ControlStatus.IMPLEMENTED).length /
        ISO27001_CONTROLS.length) * 100);
    const soc2Score = Math.round((SOC2_CONTROLS.filter((c) => c.status === ControlStatus.IMPLEMENTED).length /
        SOC2_CONTROLS.length) * 100);
    const wcagScore = Math.round((WCAG_CRITERIA.filter((c) => c.status === "pass").length /
 ...
 */
exports.monthlyComplianceReviewScheduler = (0, scheduler_1.onSchedule)({
    schedule: "0 9 1 * *", // 9 AM on first of month
    region: "europe-west3",
    timeoutSeconds: 540,
}, async () => {
    const db = (0, firestore_1.getFirestore)();
    v2_1.logger.info("Running monthly compliance review");
    // Check for controls needing review
    const now = firestore_1.Timestamp.now();
    const controlsNeedingReview = ISO27001_CONTROLS.filter((c) => c.nextReview.toMillis() <= now.toMillis());
    if (controlsNeedingReview.length > 0) {
        v2_1.logger.warn(`${controlsNeedingReview.length} ISO 27001 controls need review`);
        ...
        await db.collection("compliance_tasks").add({
            ...
            dueDate: firestore_1.Timestamp.fromMillis(Date.now() + 7 * 24 * 60 * 60 * 1000),
    ...
    const report = {
        ...
        iso27001: {
            compliance: Math.round((ISO27001_CONTROLS.filter((c) => c.status === ControlStatus.IMPLEMENTED).length / ISO27001_CONTROLS.length) * 100),
            controlsNeedingReview: controlsNeedingReview.length,
        },
        soc2: {
            compliance: Math.round((SOC2_CONTROLS.filter((c) => c.status === ControlStatus.IMPLEMENTED).length / SOC2_CONTROLS.length) * 100),
        },
        wcag: {
            compliance: Math.round((WCAG_CRITERIA.filter((c) => c.status === "pass").length / WCAG_CRITERIA.length) * 100),
        },
        generatedAt: firestore_1.FieldValue.serverTimestamp(),
    };
    await db.collection("monthly_compliance_reports").add(report);
    v2_1.logger.info("Monthly compliance review complete");
});
/**
 ...
 * @auth admin
 */
exports.testComplianceControlV1 = (0, https_1.onCall)({
    region: "europe-west3",
    cors: true,
    enforceAppCheck: true,
}, async (request) => {
    ...
    v2_1.logger.info(`Testing ${standard} control ${controlId}`);
...
//# sourceMappingURL=auditFramework.js.map