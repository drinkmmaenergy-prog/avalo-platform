...
exports.recommendAICompanionsCallable = exports.recommendProfilesCallable = exports.updateUserInsightOnSwipeTrigger = exports.updateUserInsightOnVisitTrigger = exports.updateUserInsightOnMessageTrigger = void 0;
 ...
 */
exports.updateUserInsightOnMessageTrigger = (0, firestore_1.onDocumentCreated)({
 ...
 */
exports.updateUserInsightOnVisitTrigger = (0, firestore_1.onDocumentCreated)({
 ...
 */
exports.updateUserInsightOnSwipeTrigger = (0, firestore_1.onDocumentCreated)({
 ...
 * Update user insight
 */
async function updateUserInsight(userId, eventType) {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (eventType === "message_sent") {
            updates.totalMessages = firestore_2.FieldValue.increment(1);
        }
        else if (eventType === "profile_visit") {
            updates.totalVisits = firestore_2.FieldValue.increment(1);
        }
        else if (eventType === "like") {
            updates.totalLikes = firestore_2.FieldValue.increment(1);
        }
 ...
 */
const RecommendProfilesSchema = zod_1.z.object({
    maxDistance: zod_1.z.number().min(1).max(1000).optional().default(50),
    ageRange: zod_1.z.object({
        min: zod_1.z.number().min(18).max(99),
        max: zod_1.z.number().min(18).max(99),
    }).optional(),
    limit: zod_1.z.number().min(1).max(50).optional().default(20),
});
exports.recommendProfilesCallable = (0, https_1.onCall)({ region: "europe-west3" }, async (request) => {
    ...
    try {
        ...
        const scoredProfiles = candidatesSnapshot.docs
            ...
            if (userLocation && profile.location && maxDistance) {
                const distance = calculateDistance(userLocation.latitude, userLocation.longitude, profile.location.latitude, profile.location.longitude);
            ...
            if (insight?.topInterests && profile.interests) {
                const commonInterests = insight.topInterests.filter((interest) => profile.interests.includes(interest));
        ...
        throw new https_1.HttpsError("internal", `Failed to recommend profiles: ${error.message}`);
    }
});
/**
 * Recommend AI companions callable
 */
const RecommendAISchema = zod_1.z.object({
    limit: zod_1.z.number().min(1).max(20).optional().default(10),
});
exports.recommendAICompanionsCallable = (0, https_1.onCall)({ region: "europe-west3" }, async (request) => {
    const uid = request.auth?.uid;
    if (!uid) {
        throw new https_1.HttpsError("unauthenticated", "User must be authenticated");
    }
    ...
    try {
        ...
        throw new https_1.HttpsError("internal", `Failed to recommend AI companions: ${error.message}`);
    }
});
/**
