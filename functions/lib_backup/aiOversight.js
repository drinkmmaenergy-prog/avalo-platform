...
exports.getAIOversightStatsV1 = exports.resolveModerationItemV1 = exports.getModerationQueueV1 = exports.analyzeContentV1 = exports.ModerationAction = exports.RiskLevel = exports.RiskCategory = exports.ContentType = void 0;
...
(function (RiskCategory) {
    RiskCategory["SCAM_FRAUD"] = "scam_fraud";
    RiskCategory["HARASSMENT"] = "harassment";
    RiskCategory["NSFW"] = "nsfw";
    RiskCategory["HATE_SPEECH"] = "hate_speech";
    RiskCategory["SPAM"] = "spam";
    RiskCategory["SELF_HARM"] = "self_harm";
    RiskCategory["VIOLENCE"] = "violence";
    RiskCategory["PII_LEAK"] = "pii_leak";
    RiskCategory["MINOR_SAFETY"] = "minor_safety";
    RiskCategory["FINANCIAL_ABUSE"] = "financial_abuse";
...
(function (ModerationAction) {
    ...
    ModerationAction["SHADOW_BAN"] = "shadow_ban";
    ModerationAction["BLOCK"] = "block";
    ModerationAction["ESCALATE"] = "escalate";
    ModerationAction["AUTO_DELETE"] = "auto_delete";
 ...
 */
async function analyzeWithClaude(content, context) {
...
Provide your analysis in JSON format.`;
    try {
        ...
        if (!response.ok) {
            ...
            v2_1.logger.error("Claude API error:", error);
            throw new Error(`Claude API failed: ${response.status}`);
        }
        const data = await response.json();
        const latency = Date.now() - startTime;
        v2_1.logger.info(`Claude analysis completed in ${latency}ms`);
        // Parse Claude's response
        const claudeResponse = data.content[0].text;
        // Extract JSON from response (Claude might wrap it in markdown)
        const jsonMatch = claudeResponse.match(/\{[\s\S]*\}/);
        if (!jsonMatch) {
            v2_1.logger.error("Failed to parse Claude response:", claudeResponse);
        ...
        v2_1.logger.error("Claude API call failed:", error);
 ...
 */
function determineModerationAction(analysis, contextFactors) {
 ...
 */
exports.analyzeContentV1 = (0, https_1.onCall)({
    region: "europe-west3",
    cors: true,
    ...
    v2_1.logger.info(`AI analyzing ${contentType} from user ${userId}`);
    const startTime = Date.now();
    // Get user context
    ...
    let riskLevel;
    if (adjustedRiskScore >= 76)
        riskLevel = RiskLevel.CRITICAL;
    else if (adjustedRiskScore >= 51)
        riskLevel = RiskLevel.WARNING;
    else if (adjustedRiskScore >= 21)
        riskLevel = RiskLevel.CAUTION;
    else
        riskLevel = RiskLevel.SAFE;
    // Determine moderation action
    const recommendation = determineModerationAction({ ...claudeAnalysis, riskScore: adjustedRiskScore }, contextFactors);
    const requiresHumanReview = recommendation === ModerationAction.REVIEW ||
    ...
    if (requiresHumanReview) {
        ...
        v2_1.logger.info(`Added ${result.contentId} to moderation queue (priority ${queueItem.priority})`);
    }
    ...
    v2_1.logger.info(`AI analysis complete: ${riskLevel} risk (${adjustedRiskScore}/100), ${recommendation}, ${latency}ms`);
    return result;
});
/**
 * Execute moderation action
 */
async function executeModerationAction(contentId, action, userId, source) {
    const db = (0, firestore_1.getFirestore)();
    v2_1.logger.info(`Executing ${action} on ${contentId} by ${source}`);
    // Log action
 ...
 */
exports.getModerationQueueV1 = (0, https_1.onCall)({
    region: "europe-west3",
    cors: true,
    enforceAppCheck: true,
}, async (request) => {
 ...
 * @auth moderator/admin
 */
exports.resolveModerationItemV1 = (0, https_1.onCall)({
    region: "europe-west3",
    cors: true,
    enforceAppCheck: true,
}, async (request) => {
    ...
    v2_1.logger.info(`Moderator ${moderatorId} resolved ${queueId} with ${action}`);
    return { success: true };
});
/**
 ...
 * @auth admin
 */
exports.getAIOversightStatsV1 = (0, https_1.onCall)({
    region: "europe-west3",
    cors: true,
    enforceAppCheck: true,
}, async (request) => {
    ...
    const startDate = firestore_1.Timestamp.fromMillis(Date.now() - days * 24 * 60 * 60 * 1000);
...
//# sourceMappingURL=aiOversight.js.map