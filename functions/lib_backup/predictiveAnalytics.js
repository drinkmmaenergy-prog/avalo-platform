...
exports.exportMetricsV1 = exports.generatePredictionsV1 = void 0;
 ...
 */
exports.generatePredictionsV1 = (0, https_1.onCall)({ region: "europe-west3", memory: "2GiB", timeoutSeconds: 540 }, async (request) => {
    ...
    const schema = zod_1.z.object({
        window: zod_1.z.enum(["7d", "30d", "90d", "365d"]).optional().default("30d"),
        batchSize: zod_1.z.number().min(10).max(1000).optional().default(100),
        userId: zod_1.z.string().optional(), // Generate for specific user
    });
    ...
    v2_1.logger.info("Starting prediction generation", { uid, window, batchSize, userId });
    try {
        if (userId) {
        ...
        const cutoffDate = firestore_1.Timestamp.fromMillis(Date.now() - windowMs);
        ...
        for (const userDoc of activeUsers.docs) {
            try {
                ...
                v2_1.logger.error(`Failed to generate prediction for user ${userDoc.id}`, { error });
                errors++;
            }
        }
        // Store predictions
        ...
        await batch.commit();
        v2_1.logger.info(`Prediction generation complete: ${processed} succeeded, ${errors} failed`);
        return {
            success: true,
            predictions,
            count: processed,
        ...
        v2_1.logger.error("Error generating predictions", { error, uid });
        throw new https_1.HttpsError("internal", `Failed to generate predictions: ${error.message}`);
    }
});
/**
 * Generate prediction for a single user
 */
async function generateUserPrediction(userId, window) {
    v2_1.logger.info(`Generating prediction for user: ${userId}`);
    // Fetch all required data in parallel
    const [userDoc, trustDoc, behaviorDoc, messageDoc, transactionsSnapshot, sessionsSnapshot,] = await Promise.all([
    ...
    // Extract features
    const features = extractFeatures(userData, trustDoc, behaviorDoc, messageDoc, transactionsSnapshot, sessionsSnapshot, accountAgeDays);
 ...
 */
function extractFeatures(userData, trustDoc, behaviorDoc, messageDoc, transactionsSnapshot, sessionsSnapshot, accountAgeDays) {
    ...
    const totalRevenue = transactions.reduce((sum, tx) => sum + (tx.amountTokens || 0), 0);
 ...
 */
function predictLTV(features, accountAgeDays) {
    if (accountAgeDays < 7) {
 ...
 */
function calculateARPU(features, accountAgeDays) {
    if (accountAgeDays < 1)
        return 0;
 ...
 */
function predictNextPurchase(features) {
 ...
 */
function getChurnRisk(probability) {
    if (probability >= 0.75)
        return "critical";
    if (probability >= 0.5)
        return "high";
 ...
 */
function generateRecommendations(features, churnProbability) {
 ...
 * Determine user segment
 */
function getUserSegment(totalRevenue, purchaseCount) {
    if (totalRevenue > 10000)
        return "whale"; // High spenders
    if (totalRevenue > 1000)
        return "dolphin"; // Medium spenders
    if (totalRevenue > 100)
        return "minnow"; // Low spenders
    if (purchaseCount > 0)
        return "paying"; // Has purchased
...
function calculateStreak(sessions) {
    ...
    for (let i = 1; i < sessions.length; i++) {
        const currentDate = new Date(sessions[i].startedAt.toMillis())
            .toISOString()
            .split("T")[0];
        const dayDiff = Math.round((new Date(previousDate).getTime() - new Date(currentDate).getTime()) /
            (24 * 60 * 60 * 1000));
        if (dayDiff === 1) {
            streak++;
 ...
 */
exports.exportMetricsV1 = (0, https_1.onCall)({ region: "europe-west3", memory: "1GiB", timeoutSeconds: 540 }, async (request) => {
    const uid = request.auth?.uid;
    if (!uid) {
        throw new https_1.HttpsError("unauthenticated", "User must be authenticated");
    }
    ...
    v2_1.logger.info("Exporting analytics metrics", { uid, format, dateRange });
    try {
        const today = new Date().toISOString().split("T")[0];
        ...
        v2_1.logger.info("Metrics exported successfully", { count: predictions.length });
        return {
            success: true,
            metrics,
            format,
        ...
        v2_1.logger.error("Error exporting metrics", { error, uid });
        throw new https_1.HttpsError("internal", `Failed to export metrics: ${error.message}`);
    }
});
/**
 * Calculate segment metrics
 */
function calculateSegmentMetrics(predictions) {
    const segments = {};
 ...
 */
function calculateChurnMetrics(predictions) {
 ...
 */
function calculateRevenueMetrics(predictions) {
 ...
 */
function calculateEngagementMetrics(predictions) {
    const avgEngagement = predictions.reduce((sum, p) => sum + p.features.engagementScore, 0) / predictions.length;
