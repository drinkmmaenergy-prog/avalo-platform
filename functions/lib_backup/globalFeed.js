...
exports.refreshGlobalFeedScheduled = exports.invalidateFeedCacheV1 = exports.getGlobalFeedV1 = void 0;
 ...
 */
exports.getGlobalFeedV1 = (0, https_1.onCall)({ region: "europe-west3", memory: "512MiB" }, async (request) => {
    ...
    const schema = zod_1.z.object({
        ...
        language: zod_1.z.string().length(2), // ISO 639-1 code (e.g., "en", "pl")
        page: zod_1.z.number().min(0).max(MAX_FEED_PAGES).optional().default(0),
        limit: zod_1.z.number().min(5).max(50).optional().default(FEED_PAGE_SIZE),
        filters: zod_1.z.object({
            tags: zod_1.z.array(zod_1.z.string()).optional(),
            minTrustScore: zod_1.z.number().min(0).max(1000).optional(),
            timeWindow: zod_1.z.enum(["1h", "6h", "24h", "7d", "30d"]).optional(),
        }).optional(),
    });
    ...
    v2_1.logger.info(`Fetching global feed: ${cacheKey}`, { uid });
    try {
        // Try to get from cache
        const cachedFeed = await (0, cacheManager_1.getCached)(cacheKey, async () => {
        ...
        }, FEED_CACHE_TTL, [`feed`, `region:${params.region}`, `lang:${params.language}`]);
        // Filter out posts from blocked users
        const userDoc = await db.collection("users").doc(uid).get();
        const blockedUsers = userDoc.data()?.blockedUsers || [];
        const filteredFeed = cachedFeed.filter(post => !blockedUsers.includes(post.authorId));
        // Apply pagination
        ...
        v2_1.logger.error("Error fetching global feed", { error, uid, params });
        throw new https_1.HttpsError("internal", `Failed to fetch feed: ${error.message}`);
    }
});
/**
 ...
 */
async function fetchAndRankPosts(params) {
    v2_1.logger.info("Fetching posts from Firestore", { params });
    // Calculate time window
    const timeWindowMs = getTimeWindowMs(params.filters?.timeWindow || "24h");
    const cutoffTime = firestore_1.Timestamp.fromMillis(Date.now() - timeWindowMs);
    ...
    if (postsSnapshot.empty) {
        v2_1.logger.info("No posts found for query", { params });
        return [];
    }
    v2_1.logger.info(`Found ${postsSnapshot.size} posts, ranking...`);
    ...
    for (const doc of postsSnapshot.docs) {
        ...
        const feedScore = Math.round((engagementScore * 0.6) +
            (trustComponent * 0.4) +
            recencyBoost);
    ...
    v2_1.logger.info(`Ranked ${rankedPosts.length} posts`);
    return rankedPosts;
}
/**
 * Fetch trust scores for multiple users (batched)
 */
async function fetchTrustScores(userIds) {
    const trustScores = {};
    if (userIds.length === 0)
        return trustScores;
...
function calculateEngagementScore(engagement) {
    const rawScore = (engagement.likes * 1) +
 ...
 */
async function logFeedView(userId, region, language, postCount) {
    try {
        await db.collection("analyticsEvents").add({
            ...
            serverTimestamp: firestore_1.Timestamp.now(),
            processed: false,
            createdAt: firestore_1.Timestamp.now(),
        });
    }
    catch (error) {
        v2_1.logger.warn("Failed to log feed view", { error, userId });
    }
}
/**
 ...
 */
exports.invalidateFeedCacheV1 = (0, https_1.onCall)({ region: "europe-west3" }, async (request) => {
    const uid = request.auth?.uid;
    if (!uid) {
        throw new https_1.HttpsError("unauthenticated", "User must be authenticated");
    }
    ...
    v2_1.logger.info(`Invalidating feed cache for post: ${postId}`, { uid, region, language });
    try {
        // Invalidate specific region+language cache
        if (region && language) {
            await (0, cacheManager_1.invalidateCacheByTags)([`feed`, `region:${region}`, `lang:${language}`]);
        }
        else {
            // Invalidate all feed caches
            await (0, cacheManager_1.invalidateCacheByTags)([`feed`]);
        }
        return {
            success: true,
            postId,
        ...
        v2_1.logger.error("Error invalidating feed cache", { error, postId });
        throw new https_1.HttpsError("internal", `Failed to invalidate cache: ${error.message}`);
    }
});
/**
 ...
 */
exports.refreshGlobalFeedScheduled = (0, scheduler_1.onSchedule)({
    schedule: "*/15 * * * *", // Every 15 minutes
    region: "europe-west3",
    timeoutSeconds: 540,
    memory: "1GiB",
}, async () => {
    v2_1.logger.info("Starting scheduled feed refresh");
    try {
        ...
        for (const combo of combinations) {
            try {
                ...
                await (0, cacheManager_1.getCached)(cacheKey, async () => posts, FEED_CACHE_TTL, [`feed`, `region:${combo.region}`, `lang:${combo.language}`]);
                refreshed++;
                v2_1.logger.info(`Refreshed feed cache: ${combo.region}/${combo.language} (${posts.length} posts)`);
            }
            catch (error) {
                v2_1.logger.error(`Failed to refresh feed: ${combo.region}/${combo.language}`, { error });
                errors++;
            }
        }
        v2_1.logger.info(`Feed refresh complete: ${refreshed} succeeded, ${errors} failed`);
        // Log to engineLogs
        ...
        v2_1.logger.error("Feed refresh scheduler failed", { error });
        throw error;
    }
});
//# sourceMappingURL=globalFeed.js.map