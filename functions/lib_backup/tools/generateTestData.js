...
exports.generateTestDataCallable = void 0;
 ...
 * Admin only
 */
exports.generateTestDataCallable = (0, https_1.onCall)({ region: "europe-west3" }, async (request) => {
    ...
    if (!roles.admin) {
        throw new https_1.HttpsError("permission-denied", "Only admins can generate test data");
    }
    try {
        ...
        throw new https_1.HttpsError("internal", `Failed to generate test data: ${error.message}`);
    }
});
/**
    ...
    for (let i = 0; i < 10; i++) {
        ...
        await db.collection("users").doc(userId).set({
            ...
            verification: {
                ...
                verifiedAt: i < 5 ? firestore_1.Timestamp.now() : null,
            },
            wallet: {
                balance: 500 + i * 100,
                spent: i * 50,
            ...
            roles: {},
            banned: false,
            createdAt: firestore_1.Timestamp.fromMillis(Date.now() - (10 - i) * 24 * 60 * 60 * 1000),
 ...
 */
async function generateChats(userIds) {
    const chatIds = [];
    for (let i = 0; i < 30; i++) {
        ...
        await db.collection("chats").doc(chatId).set({
            ...
            createdAt: firestore_1.Timestamp.fromMillis(Date.now() - (30 - i) * 24 * 60 * 60 * 1000),
            updatedAt: firestore_1.Timestamp.now(),
        });
        chatIds.push(chatId);
        // Add some messages
        ...
        for (let j = 0; j < msgCount; j++) {
            await db
                ...
                createdAt: firestore_1.Timestamp.fromMillis(Date.now() - (30 - i) * 24 * 60 * 60 * 1000 + j * 60000),
            });
        }
    }
    return chatIds;
 ...
 */
async function generateTransactions(userIds) {
    const txIds = [];
    const types = [
        "token_purchase",
        "chat_payment",
    ...
    for (let i = 0; i < 15; i++) {
        ...
        if (type.includes("purchase"))
            amount = 100 + i * 50;
        else if (type.includes("payment") || type.includes("sent") || type.includes("booking"))
            amount = -(50 + i * 10);
        else if (type.includes("earned") || type.includes("received"))
            amount = 40 + i * 8;
        else if (type === "refund")
            amount = 30 + i * 5;
        await db.collection("transactions").doc(txId).set({
            ...
            createdAt: firestore_1.Timestamp.fromMillis(Date.now() - (15 - i) * 24 * 60 * 60 * 1000),
        });
        txIds.push(txId);
    }
    return txIds;
 ...
 */
async function generateAISubscriptions(userIds) {
    const subIds = [];
    ...
    for (let i = 0; i < 5; i++) {
        ...
        await db
            ...
            "aiSubscription.startedAt": firestore_1.Timestamp.fromMillis(Date.now() - i * 7 * 24 * 60 * 60 * 1000),
        });
        subIds.push(subId);
    }
    return subIds;
 ...
 */
async function generateCalendarBookings(userIds) {
    const bookingIds = [];
    for (let i = 0; i < 5; i++) {
        ...
        await db.collection("calendarBookings").doc(bookingId).set({
            ...
            status: "confirmed",
            scheduledFor: firestore_1.Timestamp.fromMillis(Date.now() + (i + 1) * 7 * 24 * 60 * 60 * 1000),
            createdAt: firestore_1.Timestamp.fromMillis(Date.now() - (5 - i) * 24 * 60 * 60 * 1000),
        });
 ...
 */
async function generateAMLFlags(userIds) {
    const flagIds = [];
    ...
    for (let i = 0; i < 3; i++) {
        ...
        await db.collection("amlFlags").doc(flagId).set({
            ...
            amount: 500 + i * 200,
            detectedAt: firestore_1.Timestamp.fromMillis(Date.now() - i * 24 * 60 * 60 * 1000),
            reviewed: false,
        });
        flagIds.push(flagId);
    }
