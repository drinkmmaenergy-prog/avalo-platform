...
describe("Discovery Ranking v2 - Scoring Algorithm", () => {
    ...
    describe("Recency Score Calculation", () => {
        test("should give max score for activity within 1 hour", () => {
            ...
            if (hoursSinceActivity < 1)
                ...
                recencyScore = RANKING_WEIGHTS.RECENCY * 0.8;
            else if (hoursSinceActivity < 72)
                recencyScore = RANKING_WEIGHTS.RECENCY * 0.5;
            else if (hoursSinceActivity < 168)
                recencyScore = RANKING_WEIGHTS.RECENCY * 0.3;
            else
                recencyScore = RANKING_WEIGHTS.RECENCY * 0.1;
    ...
    describe("Distance Score Calculation (Haversine)", () => {
        ...
        test("should give max score for distance < 5km", () => {
            ...
            if (distanceKm < 5)
                ...
                distanceScore = RANKING_WEIGHTS.DISTANCE * 0.8;
            else if (distanceKm < 50)
                distanceScore = RANKING_WEIGHTS.DISTANCE * 0.5;
            else if (distanceKm < 100)
                distanceScore = RANKING_WEIGHTS.DISTANCE * 0.3;
            else
                distanceScore = RANKING_WEIGHTS.DISTANCE * 0.1;
    ...
    describe("Interaction Depth Score", () => {
        test("should give high score for many interactions", () => {
            ...
            const interactionScore = Math.min(RANKING_WEIGHTS.INTERACTION_DEPTH, (totalMessages * 0.3 + totalLikes * 0.2 + totalViews * 0.05) / 10);
    ...
    describe("Reply Latency Score", () => {
        test("should give max score for replies under 5 minutes", () => {
            ...
            let latencyScore = 0;
            if (avgReplyMinutes < 5)
                latencyScore = RANKING_WEIGHTS.REPLY_LATENCY;
            else if (avgReplyMinutes < 30)
                latencyScore = RANKING_WEIGHTS.REPLY_LATENCY * 0.7;
            else if (avgReplyMinutes < 120)
                latencyScore = RANKING_WEIGHTS.REPLY_LATENCY * 0.4;
            else
                latencyScore = RANKING_WEIGHTS.REPLY_LATENCY * 0.1;
    ...
    describe("Signal Rollup Logic", () => {
        test("should accumulate signals correctly", () => {
            ...
            const hoursSinceActivity = (Date.now() - mostRecentActivity) / (1000 * 60 * 60);
