...
exports.getRankingsCallable = exports.rebuildRankingsScheduler = exports.getUserLoyaltyCallable = exports.claimRewardCallable = exports.awardPointsOnTx = exports.RankingPeriod = exports.BadgeType = exports.UserLevel = void 0;
...
var BadgeType;
(function (BadgeType) {
    BadgeType["EARLY_ADOPTER"] = "early_adopter";
    BadgeType["VERIFIED"] = "verified";
    BadgeType["TOP_EARNER"] = "top_earner";
    BadgeType["TOP_SPENDER"] = "top_spender";
    BadgeType["SOCIAL_BUTTERFLY"] = "social_butterfly";
    BadgeType["GENEROUS"] = "generous";
    BadgeType["POPULAR"] = "popular";
    BadgeType["STREAMER"] = "streamer";
 ...
 */
exports.awardPointsOnTx = (0, firestore_1.onDocumentCreated)({
...
async function awardPoints(userId, points, reason) {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!loyaltyDoc.exists) {
            updates.createdAt = firestore_2.FieldValue.serverTimestamp();
            updates.badges = [];
            updates.rewardsClaimedCount = 0;
        }
        // Increment specific stats
        if (reason.includes("message")) {
            updates.messagesCount = firestore_2.FieldValue.increment(1);
        }
        if (reason.includes("tip_sent") || reason.includes("live_tip")) {
            updates.tipsGiven = firestore_2.FieldValue.increment(1);
        }
        if (reason.includes("tip_received")) {
            updates.tipsReceived = firestore_2.FieldValue.increment(1);
        }
        if (reason.includes("spent") || reason.includes("payment")) {
            updates.tokensSpent = firestore_2.FieldValue.increment(points / POINTS_CONFIG.TOKEN_SPENT);
        }
        if (reason.includes("earned")) {
            updates.tokensEarned = firestore_2.FieldValue.increment(points / POINTS_CONFIG.TOKEN_EARNED);
        }
...
function calculateLevel(points) {
    if (points >= LEVEL_THRESHOLDS[UserLevel.DIAMOND])
        return UserLevel.DIAMOND;
    if (points >= LEVEL_THRESHOLDS[UserLevel.PLATINUM])
        return UserLevel.PLATINUM;
    if (points >= LEVEL_THRESHOLDS[UserLevel.GOLD])
        return UserLevel.GOLD;
    if (points >= LEVEL_THRESHOLDS[UserLevel.SILVER])
 ...
 */
exports.claimRewardCallable = (0, https_1.onCall)({ region: "europe-west3" }, async (request) => {
    ...
    try {
        ...
        if (rewardsClaimedCount >= Object.keys(UserLevel).indexOf(level) + 1) {
            throw new https_1.HttpsError("failed-precondition", "Reward already claimed for this level");
        }
        // Credit user wallet
        const userRef = db.collection("users").doc(uid);
        await db.runTransaction(async (tx) => {
            tx.update(userRef, {
                "wallet.balance": firestore_2.FieldValue.increment(rewardAmount),
                updatedAt: firestore_2.FieldValue.serverTimestamp(),
            });
            tx.update(loyaltyRef, {
                rewardsClaimedCount: firestore_2.FieldValue.increment(1),
                lastRewardClaim: firestore_2.FieldValue.serverTimestamp(),
                updatedAt: firestore_2.FieldValue.serverTimestamp(),
            });
        ...
        if (error instanceof https_1.HttpsError)
            throw error;
        throw new https_1.HttpsError("internal", "Failed to claim reward");
    }
});
/**
 * Get user loyalty stats
 */
exports.getUserLoyaltyCallable = (0, https_1.onCall)({ region: "europe-west3" }, async (request) => {
    const uid = request.auth?.uid;
    if (!uid) {
        throw new https_1.HttpsError("unauthenticated", "User must be authenticated");
    }
    ...
    try {
        ...
        throw new https_1.HttpsError("internal", "Failed to get loyalty data");
    }
});
/**
 * Scheduled: Rebuild rankings every 24 hours
 */
exports.rebuildRankingsScheduler = (0, scheduler_1.onSchedule)({
 ...
 */
exports.getRankingsCallable = (0, https_1.onCall)({ region: "europe-west3" }, async (request) => {
    const { period, periodKey } = request.data;
    if (!period) {
        throw new https_1.HttpsError("invalid-argument", "Missing period");
    }
    try {
        ...
        throw new https_1.HttpsError("internal", "Failed to get rankings");
    }
});
