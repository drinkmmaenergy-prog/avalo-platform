/**
 * Fix multiline functions.pubsub.schedule patterns
 * Pattern:
 ...
 * 
 * Convert to:
 * export const name = onSchedule("cron", async (event) => {
 */
...
for (const filePath of files) {
  ...
  // Pattern 3: functions.pubsub\n  .schedule('cron')\n  .timeZone('tz')\n  .onRun(async () => {
  const pattern3 = /functions\.pubsub\s*\n\s*\.schedule\s*\(\s*['"]([^'"]+)['"]\s*\)\s*\n\s*\.timeZone\s*\(\s*['"]([^'"]+)['"]\s*\)\s*\n\s*\.onRun\s*\(\s*async\s*\(\s*\)\s*=>\s*\{/g;
  ...
  // Pattern 4: functions.pubsub\n  .schedule('cron')\n  .timeZone('tz')\n  .onRun(async (context) => {
  const pattern4 = /functions\.pubsub\s*\n\s*\.schedule\s*\(\s*['"]([^'"]+)['"]\s*\)\s*\n\s*\.timeZone\s*\(\s*['"]([^'"]+)['"]\s*\)\s*\n\s*\.onRun\s*\(\s*async\s*\(\s*(\w+)\s*\)\s*=>\s*\{/g;
