// ARCHIVED [G5] — legacy module with forbidden patterns. NOT exported from index.ts.
// DO NOT re-export. Canonical replacements:
//   live.ts           → canonicalChatStateMachineV3.ts (direct chat) + canonicalMultiRoomV2.ts (rooms)
//   chat/multiChatRoom.ts → canonicalMultiRoomV2.ts
//   chatMonetization.ts   → canonicalChatStateMachineV3.ts (deactivated via V9)
//   chats.ts              → canonicalChatStateMachineV3.ts (deactivated via V9)
// Forbidden patterns present: users/{uid}.wallet.balance, 80/20 split, Date.now() idempotency
// Retained for git history only. DO NOT activate.
...
import { admin, arrayRemove, arrayUnion, auth, getFirestore, increment, onCall, onSchedule, serverTimestamp, timestamp } from './runtime';
...
export const sendLiveTipCallable = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      const platformFee = Math.floor(amount * MONETIZATION_SPLITS.EVENT_TICKET.platform); // 20% platform fee
        ...
        for (const viewerId of session.viewerIds) {
          try {
            await db.runTransaction(async (tx) => {
              ...
              // Credit host (70%)
              const hostAmount = Math.floor(tokensPerTick * MONETIZATION_SPLITS.SUBSCRIPTION.earner);
