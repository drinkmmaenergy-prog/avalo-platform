...
describe("Security Operations Center", () => {
  ...
  describe("Token Drain Detection", () => {
    ...
    test("should detect system-wide token drain", () => {
      ...
      const isAnomaly =
        tokensSpentLastHour > THRESHOLDS.TOKENS_PER_HOUR_MAX;
    ...
    test("should detect per-user token drain", () => {
      ...
      const isAnomaly =
        userTokensSpentLastHour > THRESHOLDS.TOKENS_PER_USER_HOUR_MAX;
  ...
  describe("Rapid Account Creation Detection", () => {
    ...
    test("should detect account creation spike", () => {
      ...
      const isAnomaly =
        accountsCreatedLastHour > THRESHOLDS.NEW_ACCOUNTS_PER_HOUR_MAX;
    ...
    test("should detect IP-based account farming", () => {
      ...
      const isAnomaly =
        accountsFromIPLastHour > THRESHOLDS.NEW_ACCOUNTS_PER_IP_HOUR_MAX;
  ...
  describe("Incident Severity Calculation", () => {
    function calculateSeverity(
      ...
      switch (incidentType) {
        case IncidentType.UNUSUAL_TOKEN_DRAIN:
          if (magnitude > 50000) return IncidentSeverity.CRITICAL;
          if (magnitude > 20000) return IncidentSeverity.HIGH;
          if (magnitude > 10000) return IncidentSeverity.MEDIUM;
          return IncidentSeverity.LOW;

        case IncidentType.RAPID_ACCOUNT_CREATION:
          if (magnitude > 200) return IncidentSeverity.CRITICAL;
          if (magnitude > 100) return IncidentSeverity.HIGH;
          if (magnitude > 50) return IncidentSeverity.MEDIUM;
          return IncidentSeverity.LOW;

        case IncidentType.LARGE_TRANSACTION:
          if (magnitude > 50000) return IncidentSeverity.HIGH;
          if (magnitude > 20000) return IncidentSeverity.MEDIUM;
          return IncidentSeverity.LOW;
    ...
    test("should assign CRITICAL severity to massive token drain", () => {
      const severity = calculateSeverity(
        IncidentType.UNUSUAL_TOKEN_DRAIN,
        60000
      );
    ...
    test("should assign CRITICAL severity to account creation spike", () => {
      const severity = calculateSeverity(
        IncidentType.RAPID_ACCOUNT_CREATION,
        250
      );
      expect(severity).toBe(IncidentSeverity.CRITICAL);
    });
  ...
  describe("Anomaly Score Aggregation", () => {
    interface AnomalyMetrics {
      tokenDrain: boolean;
      accountSpike: boolean;
      apiAbuse: boolean;
      contentFlood: boolean;
    }

    function calculateAnomalyScore(metrics: AnomalyMetrics): number {
      let score = 0;
      if (metrics.tokenDrain) score += 40;
      if (metrics.accountSpike) score += 30;
      if (metrics.apiAbuse) score += 20;
      if (metrics.contentFlood) score += 10;
      return score;
    }
