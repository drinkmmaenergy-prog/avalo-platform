 ...
 */
export function detectSpam(content: string): { isSpam: boolean; risk: RiskLevel; matches: string[] } {
 ...
 */
export function detectEscortContent(content: string): { detected: boolean; risk: RiskLevel; matches: string[] } {
  const matches: string[] = [];
  let matchCount = 0;
 ...
 */
export function detectPII(content: string): { detected: boolean; risk: RiskLevel; types: string[] } {
  const types: string[] = [];

  if (PII_PATTERNS[0].test(content)) types.push("phone");
  if (PII_PATTERNS[1].test(content)) types.push("email");
  if (PII_PATTERNS[2].test(content)) types.push("address");
...
/**
 * Check user behavior patterns for fraud
 */
...
export function assessUserRisk(behavior: UserBehaviorData): { risk: RiskLevel; reasons: string[] } {
 ...
 */
export function detectBotBehavior(timestamps: Timestamp[]): { isBot: boolean; risk: RiskLevel } {
...
export interface ContentScanResult {
  ...
  details: {
    ...
    escort?: ReturnType<typeof detectEscortContent>;
