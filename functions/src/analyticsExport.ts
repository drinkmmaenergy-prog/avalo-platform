...
class BigQuery {
  ...
  async createDataset(name: string, options?: any) {
    return {};
  }
  dataset(name: string) {
    return {
      ...
      async createTable(name: string, options?: any) {
        return {};
      },
      table: (name: string) => ({
        insert: async (rows: any[], options?: any) => {},
      }),
    };
  }
}
...
export const exportAnalyticsScheduler = onSchedule(
  ...
  async (event) => {
    ...
    try {
      ...
      logger.info(`Exporting ${events.length} events to BigQuery`);
      ...
      logger.info(`Analytics export completed: ${events.length} events in ${duration}ms`);
 ...
 */
async function exportToBigQuery(events: any[]): Promise<void> {
  try {
    ...
    logger.info(`Successfully inserted ${rows.length} rows to BigQuery`);
  } catch (error: any) {
    if (error.name === "PartialFailureError") {
      logger.error("Partial failure in BigQuery insert:", error.errors);
 ...
 */
async function exportToGCS(events: any[]): Promise<void> {
  ...
  logger.info(`Exported ${events.length} events to gs://${bucket.name}/${filename}`);
}
...
async function ensureBigQuerySetup(): Promise<void> {
  try {
    ...
    logger.warn("BigQuery setup check failed:", error.message);
 ...
 */
async function handleExportFailure(error: any): Promise<void> {
  try {
    ...
    logger.info(`Moved ${failedSnapshot.size} events to dead letter queue`);
  } catch (dlqError: any) {
    logger.error("Dead letter queue failed:", dlqError);
  }
}
...
export const cleanupAnalyticsEventsScheduler = onSchedule(
  ...
  async (event) => {
    logger.info("Analytics cleanup started");
