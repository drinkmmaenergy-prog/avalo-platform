...
export interface BaseTokenPriceConfig {
  /** Base price in reference currency (USD) */
  priceUSD: number;
  
  /** Reference currency code */
  referenceCurrency: 'USD' | 'USD';
...
/**
 * Base token price in USD (FIXED, only changeable via admin 2-key)
 * Example: €0.25 per token
 */
export const BASE_TOKEN_PRICE_USD = 0.25;
...
export const SUPPORTED_CURRENCIES = {
  ...
  USD: { name: 'Polish Złoty', symbol: 'zł', decimalPlaces: 2, countries: ['PL'] },
...
export const VAT_RULES_BY_COUNTRY: Record<string, { rate: number; included: boolean; jurisdiction: string }> = {
  ...
  FR: { rate: MONETIZATION_SPLITS.EVENT_TICKET.platform, included: true, jurisdiction: 'France VAT' },
  ...
  AT: { rate: MONETIZATION_SPLITS.EVENT_TICKET.platform, included: true, jurisdiction: 'Austria VAT' },
  ...
  BG: { rate: MONETIZATION_SPLITS.EVENT_TICKET.platform, included: true, jurisdiction: 'Bulgaria VAT' },
  
  // Other countries - typically tax excluded
  US: { rate: 0.00, included: false, jurisdiction: 'US (varies by state)' },
  GB: { rate: MONETIZATION_SPLITS.EVENT_TICKET.platform, included: true, jurisdiction: 'UK VAT' },
