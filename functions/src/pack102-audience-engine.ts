 ...
 * Called when someone clicks earner's public profile link
 */
export async function logExternalVisit(
  request: LogExternalVisitRequest
): Promise<LogExternalVisitResponse> {
  const { earnerId, platform, utmSource, utmMedium, utmCampaign, visitMetadata } = request;
  ...
  // Check if earner exists and has public profile enabled
  ...
  // Anti-spam check: Rate limit visits per earner
  ...
  if (recentVisitsSnap.data().count >= SPAM_THRESHOLDS.MAX_VISITS_PER_CREATOR_PER_HOUR) {
    logger.warn(`[AudienceGrowth] Spam detected: Too many visits for earner ${earnerId}`);
  ...
  logger.info(`[AudienceGrowth] Logged visit: earner=${earnerId}, platform=${platform}`);
 ...
 * Called when new user follows the earner
 */
export async function markAttributionFollower(
  earnerId: string,
  userId: string
): Promise<void> {
  // Find most recent attribution for this earner/user combo
  ...
  logger.info(`[AudienceGrowth] Marked follower: earner=${earnerId}, user=${userId}`);
}
...
export async function markAttributionPayer(
  ...
  // Find attribution for this earner/user combo
  const attributionSnap = await db
    .collection('external_audience_attribution')
    .where('earnerId', '==', earnerId)
    .where('userId', '==', userId)
  ...
  logger.info(`[AudienceGrowth] Marked payer: earner=${earnerId}, user=${userId}`);
}
...
export async function rebuildAudienceAttributionDaily(date: Date): Promise<void> {
  ...
  attributionsSnap.forEach((doc) => {
    ...
    const key = attr.earnerId;

    let metrics = metricsMap.get(key);
    if (!metrics) {
      metrics = {
        earnerId: attr.earnerId,
        visits: 0,
        signups: 0,
        follows: 0,
  ...
  for (const [earnerId, metrics] of Array.from(metricsMap.entries())) {
    const docId = `${earnerId}_${dateStr.replace(/-/g, '')}`;
  ...
  logger.info(`[AudienceGrowth] Rebuilt attribution for ${dateStr}: ${metricsMap.size} earners`);
}

/**
 * Get audience growth metrics for a earner
