...
describe("AI Companions - Daily Limit Reset Logic", () => {
  ...
  describe("Timestamp Conversion", () => {
    test("should handle Firestore timestamp with toDate method", () => {
      ...
      const lastReset =
        typeof firestoreTimestamp.toDate === "function"
          ...
          : new Date((firestoreTimestamp as any)._seconds * 1000);
    ...
    test("should handle Firestore timestamp without toDate method (raw object)", () => {
      ...
      // Test the conversion logic
      const lastReset =
        typeof (firestoreTimestamp as any).toDate === "function"
          ? (firestoreTimestamp as any).toDate()
          : new Date((firestoreTimestamp as any)._seconds * 1000);
  ...
  describe("Subscription Tier Logic", () => {
    test("should skip limit check for Plus tier", () => {
      const subscription: Partial<AISubscription> = {
        tier: "Plus",
        status: "active",
  ...
  describe("Error Handling", () => {
    test("should handle malformed timestamp gracefully", () => {
      const badTimestamp: any = { invalid: true };

      // The function should handle this without crashing
      const lastReset =
        typeof badTimestamp.toDate === "function"
          ? badTimestamp.toDate()
    ...
    test("should handle NaN in timestamp conversion", () => {
      const badTimestamp: any = { _seconds: "not a number" };

      const lastReset = new Date(
        isNaN(badTimestamp._seconds * 1000)
          ? 0
